...
public sealed class ProcessExecutor : IProcessExecutor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // User requested cancellation - kill process immediately
            try
            {
            ...
            _logger.LogInformation(
                "Process cancelled by user: {FileName} {Arguments}",
                fileName,
                arguments);

            return (-1, outputBuilder.ToString(), "Process cancelled by user");
        }
